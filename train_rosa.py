"""Training script for ROSA language model."""
...
from decoder_pytorch import ROSA, configure_tf32, model_summary
...
def train(config_path: str, resume_checkpoint: Optional[str] = None):
    ...
    model = ROSA(
        ...
        rosa_state_cap=config.get("rosa_state_cap", 65536),
        ...
        max_seq_len=config.get("seq_len", 2048),
    ...
    run_dir = Path(config.get("run_dir", "runs/rosa"))
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Train ROSA language model",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--config", type=str, default="configs/rosa.yaml", help="Path to config file"
    )
    parser.add_argument(
